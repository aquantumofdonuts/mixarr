...
// Created For You playlist types (Weekly Jams, Weekly Exploration, etc.)
export type CreatedForYouPlaylistType = 'weekly-jams' | 'weekly-exploration' | 'top-discoveries' | 'top-missed-recordings';

interface CreatedForYouPlaylist {
  title: string;
  identifier: string;
  source_patch: string;
  date: string;
...
interface CreatedForYouResponse {
  playlists: Array<{
    playlist: {
      identifier: string;
      title: string;
...
export class ListenBrainzService {
  ...
  /**
   * Get "Created For You" playlists (Weekly Jams, Weekly Exploration, etc.)
   * These are personalized playlists generated by ListenBrainz
   */
  async getCreatedForYouPlaylists(): Promise<{
    playlists: Array<CreatedForYouPlaylist>;
    playlist_count: number;
  }> {
    const response = await this.request<CreatedForYouResponse>(
      `/1/user/${this.username}/playlists/createdfor`
    );

    return {
      playlists: response.playlists.map(p => ({
        title: p.playlist.title,
        identifier: p.playlist.identifier.replace('https://listenbrainz.org/playlist/', ''),
        source_patch: p.playlist.extension['https://musicbrainz.org/doc/jspf#playlist'].additional_metadata.algorithm_metadata.source_patch,
        date: p.playlist.date,
        expires_at: p.playlist.extension['https://musicbrainz.org/doc/jspf#playlist'].additional_metadata.expires_at,
      })),
      playlist_count: response.playlist_count,
    };
  }
  ...
  /**
   * Get the latest playlist of a specific type from "Created For You"
   * @param playlistType - The type of playlist to find (weekly-jams, weekly-exploration, etc.)
   */
  async getLatestCreatedForYouPlaylist(playlistType: CreatedForYouPlaylistType): Promise<{
    title: string;
    playlistId: string;
    tracks: Array<{ artist_name: string; artist_mbid?: string; title: string }>;
  } | null> {
    const { playlists } = await this.getCreatedForYouPlaylists();
    
    // Find the most recent playlist matching the type
    const matchingPlaylist = playlists.find(p => p.source_patch === playlistType);
    ...
    const fullPlaylist = await this.getPlaylist(matchingPlaylist.identifier);
    
    return {
      title: matchingPlaylist.title,
      playlistId: matchingPlaylist.identifier,
